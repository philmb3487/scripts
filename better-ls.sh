...
##
##	BETTER LS (overrides 'ls' command, uses /usr/local/bin/gls)
##
##	Modifications:
##	- Colorized
...
then
	...
	files=$(/usr/local/bin/gls -U * 2> /dev/null | wc -l)	
	if [ "$files" != "0" ]
	then 
		## List implied . and .., visible folders, then visible files
		/usr/local/bin/gls -d {.,..,*} -lA --color=auto --human-readable \
		...
		then
			echo ""
			/usr/local/bin/gls -d .!(|.) -l --color=auto --hide='..' \
...
## IF ARGUMENTS PASSED -> run standard ls but with some tweaks (eg: colors)		
	...
	/usr/local/bin/gls -la --color=auto --human-readable --time-style=long-iso \
		--group-directories-first "$@";	
fi
